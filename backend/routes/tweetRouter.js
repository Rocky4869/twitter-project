...
tweetRouter.get('/:username', async (req, res) => {
    ...
    if (!username) {
        return res.status(404).send({ 'success': false, 'message': `${username}'s tweets not found` });
    };
    const tweets = await Tweet.find({ 'user': user._id }).populate('user').sort({ 'last_modified': -1 });
    const result = [];
...
tweetRouter.post('/like', async (req, res) => {
    ...
    const tweet_info = await Tweet.findById(tweetId, async function(err, docs) {
        ...
        if (err) {
            return res.status(404).send({ 'success': false, 'message': 'tweets not found' });
        }
